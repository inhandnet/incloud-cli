...
func NewCmdExecRestore(f *factory.Factory) *cobra.Command {
  ...
  incloud device exec restore-defaults 507f1f77bcf86cd799439011 --yes`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if len(ids) > 1 {
				return bulkInvokeMethod(cmd, f, client, actx.Host, ids, "nezha_restore_to_defaults", nil)
			}
			return invokeMethod(cmd, f, client, actx.Host, ids[0], "nezha_restore_to_defaults", 30, nil)
		},
	}
