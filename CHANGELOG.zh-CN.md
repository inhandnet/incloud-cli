...
### 设备配置 Schema
- **`device config schema products`** — 发现设备支持的配置产品，支持去重和按产品作用域分组
...
### 知识库
- **`knowledge` 命令组** — 搜索设备文档、获取 AI 生成的解答
  - `knowledge search` — 搜索知识库，支持按设备型号过滤和查询改写
  - `knowledge ask` — 基于知识库获取 AI 解答

### 设备日志
- **`device log local`** — 获取设备本地存储的日志（补充现有的 `device log remote`）

## 改进

- **时间参数更灵活** — `--after` / `--before` 参数现在支持本地时间（`2025-01-01T08:00:00`）和仅日期格式（`2025-01-01`），不再需要手动转换为 UTC，CLI 会自动处理
- **AI 工具兼容性** — `--limit` 参数新增 `--size` 隐藏别名，AI 工具可以直接使用与 API 一致的参数名
